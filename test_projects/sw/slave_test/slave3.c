...
#define BRAM_CTRL 0x43c00000
#define BRAM_DATA 0x43c10000
...
int main() {
    ...
    unsigned int* va_bram_ctrl  = mmap(NULL, 65535, PROT_READ | PROT_WRITE, MAP_SHARED, dh, BRAM_CTRL); // Memory map source address
    unsigned int* va_bram_data  = mmap(NULL, 65535, PROT_READ | PROT_WRITE, MAP_SHARED, dh, BRAM_DATA); // Memory map source address
    ...
    *va_bram_data = 0;
    for(i=0;i< CP_W ;i++){
        *va_bram_ctrl = (i << 8) | 0x11;  // write
    }
   
    // read bram
    printf("the initial bram data is \n");
    for(i=0;i<CP_W;i++){
        *va_bram_ctrl = (i << 8) | 0x01;  // read
        printf("%d ", *va_bram_data);;
    }
    printf("\n");
